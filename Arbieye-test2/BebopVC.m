...
#import "BebopProcessedVideoView.h"
...
int status=0; //1:hukan,2:chokan,3:yoko
...
- (BOOL)bebopDrone:(BebopDrone*)bebopDrone configureDecoder:(ARCONTROLLER_Stream_Codec_t)codec{
    //[_opencvView configureDecoder:codec];
...
-(void)stabilize{
    int currentStatus = [_videoView throwCurrentStatus];
    NSLog(@"stabilizing...currentStatusis:%d",currentStatus);
}
...
-(IBAction)senkaiR:(id)sender{
    [_bebopDrone setFlag:1];
    [_bebopDrone setRoll:30];
    [_bebopDrone setYaw:-50];
...
-(IBAction)testMission:(id)sender{
    [_bebopDrone setFlag:1];
    [_bebopDrone setPitch:50];
